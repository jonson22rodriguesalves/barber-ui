 ...
 * 
 * Responsável por:
 * - Exibir agendamentos em formato de tabela
 * - Permitir adição de novos agendamentos
 * - Gerenciar exclusão de agendamentos
...
export class ScheduleCalendarComponent implements OnDestroy, AfterViewInit, OnChanges {
  ...
  private subscription?: Subscription // Gerenciamento de subscriptions RxJS

  private _selected: Date = new Date(); // Data selecionada internamente
  ...
  addingSchedule: boolean = false // Controla visibilidade do formulário
  newSchedule: SaveScheduleModel = { startAt: undefined, endAt: undefined, clientId: undefined } // Modelo do novo agendamento
  clientSelectFormControl = new FormControl() // Controle do select de clientes

  // Inputs do componente
  @Input() monthSchedule!: ScheduleAppointementMonthModel // Dados do mês
  @Input() clients: SelectClientModel[] = [] // Lista de clientes
  ...
  /**
   * Getter/Setter para data selecionada
   * Emite evento quando alterada
   */
  get selected(): Date {
   ...
   * @param form Referência ao formulário NgForm
   */
  ...
  /**
   * Solicita confirmação para exclusão de agendamento
  ...
  /**
   * Atualiza horário de término quando o de início é alterado
   * @param time Novo horário selecionado
   */
  ...
  /**
   * Constrói/atualiza a tabela de agendamentos
